...
async function loadState(sql: NeonQueryFunction<false, false>, user: AppUser) {
  ...
  return {
    ...
    profiles: (profileRows as Array<Record<string, string>>).map((row) => ({ id: row.id, email: row.email, name: row.name, relationship: row.relationship, status: row.status, role: row.id === user.id ? user.role : row.role, categoryIds: memberships.filter((item) => item.profile_id === row.id).map((item) => item.category_id) })),
